...
- New data files (maps, dialogues, minigames) must be registered in `src/data/index.ts`. New characters need a palette entry in `src/data/characters.json` (shirt/pants/hair hex colors remapped from the source spritesheet's colors — see `src/core/characters.ts`); edit visually via `tools/character-builder/` rather than hand-picking hex codes.
